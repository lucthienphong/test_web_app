...
/****** Object:  StoredProcedure [dbo].[tblJob_SelectAll]    Script Date: 05/12/2015 10:36:42 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	;WITH T AS(
		select TOP 100 PERCENT ROW_NUMBER() OVER(ORDER BY	CASE @SortColumn WHEN N'0' THEN  c.Code END ASC,
			...
			where (c.Name like '%' + @Customer + '%' or c.Code like '%' + @Customer + '%')
				...
				--and j.IsClosed = 0
				and @SortType = 'A'
				and j.Status IN (SELECT LTRIM(RTRIM(Split.a.value('.', 'VARCHAR(100)'))) 'Value' 
									FROM  
									(     
										 SELECT CAST ('<M>' + REPLACE(@Status, @Delimiter, '</M><M>') + '</M>' AS XML) AS Data            
									) AS A 
	...
	--exec tblJob_SelectAll '', '', '', '', '', 0, null, null, 0, 0, 0, 0, 0, 40, '0', 'A', 'Actived'
END

GO

/****** Object:  StoredProcedure [dbo].[Report_SalesReport]    Script Date: 05/12/2015 16:34:46 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
begin
	...
	select ROW_NUMBER() OVER(ORDER BY j.JobNumber) as RowIndex, j.JobNumber, c.SAPCode, r.Code, j.JobName, j.Design, 
			...
			po.OrderNumber as PurchaseOrderNo,
			CASE WHEN j.CurrencyID <>  @BaseCurrencyID THEN ROUND((oc.TotalPrice * (1 - oc.Discount/100)) * (1 + oc.TaxPercentage/100),2)
