...
GO
/****** Object:  StoredProcedure [dbo].[Report_SalesReport]    Script Date: 06/02/2015 15:24:46 ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	select ROW_NUMBER() OVER(ORDER BY j.JobNumber) as RowIndex, j.JobNumber, c.SAPCode, r.Code, j.JobName, j.Design, 
			...
			curr.CurrencyName, ISNULL(i.RMValue, oc.RMValue) as RMValue, 
			ISNULL(dereCyl.DeReQty, 0) as DeReQty, ISNULL(dereCyl.DeReTotalPrice, 0) as DeReTotalPrice, 
			cyl.OldQty, cyl.NewQty, cyl.TotalQty, cyl.TotalPrice, 
			ISNULL(sjd.ServiceJobs, 0) as ServiceJobs, ISNULL(othC.OtherCharges, 0) as OtherCharges,
			 oc.Discount,
		...
		from tblJob j INNER JOIN
			...
			tblReferences bo ON j.BrandOwner = bo.ReferencesID 
			LEFT OUTER JOIN
			...
			(select cyl.JobID, SUM(CASE cylSt.Physical WHEN 1 THEN isnull(cyl.Quantity,1) ELSE 0 END) as DeReQty, 
				...
				group by JobID) as dereCyl ON dereCyl.JobID = j.JobID 
				LEFT OUTER JOIN
			--Lấy tổng tiền DeReChrome
			(select cyl.JobID, 
		...
		where (j.ProductTypeID = @ProductTypeID or @ProductTypeID = 0)
				...
				and j.RevNumber = 0
				and ((j.TypeOfOrder IS NOT NULL AND j.TypeOfOrder <> 'Revision') OR j.TypeOfOrder IS NULL)
		group by j.JobNumber, s.FirstName, c.SAPCode, c.Name, r.Code, j.JobName, j.Design, 
			...
			curr.CurrencyName, i.RMValue, 
			dereCyl.DeReQty, dereCyl.DeReTotalPrice, cyl.OldQty, cyl.NewQty, cyl.TotalQty, cyl.TotalPrice,
			sjd.ServiceJobs, othC.OtherCharges, 
			oc.TotalPrice, oc.Discount, i.InvoiceNo, i.InvoiceDate, i.CreatedOn, tax.TaxCode, tax.TaxPercentage,
