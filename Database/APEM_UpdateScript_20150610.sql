...
/****** Object:  StoredProcedure [dbo].[tblJob_ProgressForDeReChrome]    Script Date: 06/10/2015 08:25:53 ******/
SET ANSI_NULLS ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[tblJob_ProgressForRepro]    Script Date: 06/10/2015 08:25:55 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  StoredProcedure [dbo].[tblJob_ProgressForEngraving]    Script Date: 06/10/2015 08:25:56 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  StoredProcedure [dbo].[tblProductionSchedule_DeReChrome]    Script Date: 06/10/2015 08:29:13 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[tblProductionSchedule_Embossing]    Script Date: 06/10/2015 08:29:14 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[tblProductionSchedule_Engraving]    Script Date: 06/10/2015 09:30:34 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  StoredProcedure [dbo].[tblPurchaseOrder_Report]    Script Date: 06/10/2015 14:20:33 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- =============================================
-- Author:		<Luong Cong Trung>
-- Create date: <06-10-2015 14:20>
-- Description:	<Description,,>
-- =============================================
create PROCEDURE [dbo].[tblPurchaseOrder_Report] 
...
;with T as 
	(
		select ROW_NUMBER() OVER(ORDER BY po.PurchaseOrderID ) as RowIndex, REPLACE(LEFT(CONVERT(varchar, po.CreatedOn, 13), 6), ' ', '-') as OrderDate, 
			(Ltrim(Str(poc.Circumference)) + ' x ' + Ltrim(Str(poc.FaceWidth))) as Size, 
			COUNT(*) as SteelBase, 
			cus.Code as Cust, 
			j.JobNumber, 
			j.JobName, 
			...
			CONVERT(nvarchar(10), po.CreatedOn, 103) as CreatedOn
			from tblPurchaseOrder as po left join
				 tblJob as j on j.JobID = po.JobID left join
				 tblPurchaseOrder_Cylinder as poc on po.PurchaseOrderID = poc.PurchaseOrderID left join
				 tblCustomer as cus on j.CustomerID = cus.CustomerID left join
				 tblSupplier as s on po.SupplierID = s.SupplierID
		    where (cus.CustomerID = @CustomerID or @CustomerID = 0)
				and (s.SupplierID = @SupplierID or @SupplierID = 0)
				and (DATEDIFF(D, j.CreatedOn, @FromDate) <= 0 or @FromDate is NULL)
				and (DATEDIFF(D, j.CreatedOn, @ToDate) >= 0 or @ToDate is NULL)
			group by j.CreatedOn, poc.Circumference, poc.FaceWidth, 
			cus.Code, j.JobNumber, j.JobName, j.Design, 
			s.Name, cus.Name, po.PurchaseOrderID,
			po.CreatedOn
	...
	select *, (select COUNT(RowIndex) from T) as RowsCount
			from T
			where @PageSize = 0
	-- exec [tblPurchaseOrder_Report] 0,0,null,null,0,1000
END
