...
begin
	select distinct pr.EngravingDate as A, '!' + LEFT(DATENAME(DW, pr.EngravingDate),3) + ' ' + REPLACE(LEFT(CONVERT(varchar, pr.EngravingDate, 13), 6), ' ', '-') EngravingDate,
		...
		from (select * from tblJob as jo where jo.RevNumber = 
(
	select MAX(ji.RevNumber)
	from tblJob as ji
	where jo.JobNumber = ji.JobNumber
)) as j INNER JOIN
			...
			(Select JobID, COUNT(CylinderID) as Qty 
					...
					where sc.Protocol = 'EMG' and sCS.Action = 'Repro' and sCS.CylinderStatusName not in ('Matching')
					group by JobID) as EMGPr ON EMGPr.JobID = j.JobID LEFT OUTER JOIN
			(Select JobID, COUNT(CylinderID) as Qty 
					from tblCylinder sC INNER JOIN
						tblCylinderStatus sCS ON sc.CylinderStatusID = sCS.CylinderStatusID 
					where sc.Protocol = 'DLS' and sCS.Action = 'Repro' and sCS.CylinderStatusName not in ('Matching')
					group by JobID) as DLSPr ON DLSPr.JobID = j.JobID LEFT OUTER JOIN
			(Select JobID, COUNT(CylinderID) as Qty 
					from tblCylinder sC INNER JOIN
						tblCylinderStatus sCS ON sc.CylinderStatusID = sCS.CylinderStatusID 
					where sc.Protocol = 'Etching' and sCS.Action = 'Repro' and sCS.CylinderStatusName not in ('Matching')
					group by JobID) as EtchingPr ON EtchingPr.JobID = j.JobID LEFT OUTER JOIN
			(Select JobID, COUNT(CylinderID) as Qty 
					from tblCylinder sC INNER JOIN
						tblCylinderStatus sCS ON  sC.CylinderStatusID = sCS.CylinderStatusID
					where sc.Protocol = 'Matching' and sCS.Action = 'Repro' 
					or (sc.Protocol = 'EMG' and sCS.Action = 'Repro' and sCS.CylinderStatusName in ('Matching'))
					or (sc.Protocol = 'DLS' and sCS.Action = 'Repro' and sCS.CylinderStatusName in ('Matching'))
					or (sc.Protocol = 'Etching' and sCS.Action = 'Repro' and sCS.CylinderStatusName in ('Matching'))
