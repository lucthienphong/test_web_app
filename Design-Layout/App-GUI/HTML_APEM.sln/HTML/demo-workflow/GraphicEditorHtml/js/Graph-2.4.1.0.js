...
if (typeof STYLE_PATH === 'undefined')
    STYLE_PATH = '';
...
Graph = function (container, model, renderHint, stylesheet) {
    ...
    /*
    //DEV:check drop and move
    this.isValidDropTarget = function (source, target) {
    console.log('check : isValidDropTarget', source, target);
    return false;
    };
    */

    /*
    //DEV:check connect ok
    this.validateEdge = function (edge, source, target) {
    ...
    */
    ...
    this.isValidConnection = function (source, target) {
        //console.log('check : ', cell.edge, cell.target);
        //console.log('check : isValidConnection', source, target);
        //mxGraphHandler.prototype.setHighlightColor('red');
        if (typeof target.isSource !== 'undefined' && target.isSource !== null
            && target.isSource.toString() === '1')
            return false;
        else if (typeof source.isTarget !== 'undefined' && source.isTarget !== null
            && source.isTarget.toString() === '1')
            return false;
        else {
            if (source.edges !== null && source.edges.length > 0) {
                //console.log(source.edges);
                for (var i = 0; i < source.edges.length; i++) {
                    if (source.edges[i].target !== null &&
                        source.edges[i].target.id === source.id &&
                        source.edges[i].source !== null
                        && source.edges[i].source.id === target.id) {
                        //console.log(source.edges[i].target, source.edges[i].source);
                        return false;
                    }
                }
            }
            return (target.edge === 1 || target.edge === true) ? false : true;
        }
    };

    /*
    //DEV:connect state
    var graphvalidateEvent = this.connectionHandler.validateConnection;
    this.connectionHandler.validateConnection = function (cell1, cell2) {
        console.log(cell1, cell2);
        graphvalidateEvent.apply(this, arguments);
    };
    */

    //DEV: change edge style
    var mxConnectionHandlerMouseUp = this.connectionHandler.mouseUp;
    this.connectionHandler.mouseUp = function (sender, me) {
        //sender==Graph
        //me:mouse event
        if (this.isConnecting() === true) {
            sender.stylesheet.getDefaultEdgeStyle()[mxConstants.STYLE_EDGE] = '';

            mxConnectionHandlerMouseUp.apply(this, arguments);

            //sender.stylesheet.getDefaultEdgeStyle()[mxConstants.STYLE_EDGE] = 'orthogonalEdgeStyle';
            sender.stylesheet.getDefaultEdgeStyle()[mxConstants.STYLE_EDGE] = mxEdgeStyle.SegmentConnector;
        }
        //console.log(sender, me);
    };

    //console.log('not allow connection');
...
(function () {
    ...
    if ('contentEditable' in document.documentElement) {
        ...
        */
        ...
        mxCellEditor.prototype.stopEditing = function (cancel) {
            //console.log(cancel, this.graph.getSelectionCells());

            if (this.text2 !== null && typeof this.text2 !== 'undefined') {
    ...
    */
    ...
    else // not touchStyle
    {
        ...
        mxConnectionHandler.prototype.targetConnectImage = true;
