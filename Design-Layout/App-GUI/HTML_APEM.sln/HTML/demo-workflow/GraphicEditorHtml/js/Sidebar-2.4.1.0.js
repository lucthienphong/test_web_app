...
Sidebar.prototype.addCustomPaletteByData = function (data) {
    ...
    if (typeof data.id !== 'undefined' && data.items.length > 0) {
        ...
        if (data.id === 'truc' || data.id === 'graphictype') {
            this.addMachinery(data.id, data.title || 'undefined', data.isExpand || false, mxUtils.bind(this, function (content) {
                ...
                for (var k = 0, l = data.items.length; k < l; k++) {
                    ...
                    if (item.isEdge) {
                        ...
                        cells[0].vertex = true;
                        var elt = ui.createItem(cells, item.title || '', true);
                        ...
                        mxEvent.addListener(elt, 'click', function (evt) {
                            ...
                            if (ui.editorUi.checkEnable()) {
                                var isEdit = ui.editorUi.onBeforeUnload();


                                if (isEdit !== null && typeof isEdit !== 'undefined') {
                                    //var linknode = this;
                                    SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.Data.notifyTitle, isEdit,
                                        'confirmCancelSave',
                                        function () {
                                            ui.editorUi.resetActionNode();
                                            ui.showWork(dataid);
                                        },
                                        function () {
                                            ui.editorUi.save(ui.editorUi.editor.getOrCreateFilename(), false);
                                            ui.showWork(dataid);
                                        });
                                }
                                else {
...
Sidebar.prototype.addFoldingHandler = function (title, content, funct) {
    ...
    mxEvent.addListener(title, 'click', function (evt) {
        ...
        if (content.style.display === 'none') {
            ...
            if (content.id === 'divtruc' || content.id === 'divgraphictype') {
...
Sidebar.prototype.loadMachineryWorkflow = function (id) {
    ...
    var onload = function (req) {
        ...
        var title = '';
        var activeMachine = si.getActiveMachinery();
        title = activeMachine !== null ? $(activeMachine).text() : SweetSoftScript.ResourceText.mainworkTitle;
        ...
        if (typeof data !== 'undefined' && data.length > 0) {
            ...
            if (obj.data.length > 0) {
                ...
                EditorUi.prototype.lastParentId = obj.id;
    ...
    new mxXmlRequest(this.loadTrucPath,'','get').send(onload, onerror);
}
...
Sidebar.prototype.processShowMachinery = function (id) {
    var si = this;
    //console.log('id : ', id);
    this.showPalette('depts');
    this.editorUi.setHeightDepts();
    var obj = si.editorUi.getTrucDocument(id);
    if (typeof obj !== 'undefined') {
        ...
        var title = '';
        var activeMachine = si.getActiveMachinery();
        title = activeMachine !== null ? $(activeMachine).text() : '';
        //console.log(ed);
        // var title = source.value + ' - ' + target.value;
        this.editorUi.updateDocumentTitle(title);
        this.editorUi.editor.setFilename('Main-Work');
        var doc = mxUtils.parseXml(obj.data);
        //console.log('doc : ', doc);
        EditorUi.prototype.lastParentId = obj.id;
        EditorUi.prototype.currentId = obj.id;
...
Sidebar.prototype.showAllPalette = function () {
    var pals = this.palettes;
    //console.log('pals : ', pals, pals.length);
    if (pals !== null) {
        for (var key in pals) {
            //var obj = pals[key];
            //console.log(key, obj);
