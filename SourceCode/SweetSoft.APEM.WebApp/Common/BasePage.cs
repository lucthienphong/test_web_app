...
namespace SweetSoft.APEM.WebApp.Common
{
    public class BasePage : System.Web.UI.Page
    {
        ...
        /// </summary>
        protected MainMasterPage CURRENT_MASTERPAGES
        {
            get
            {
                try
                {
                    ...
                    return (MainMasterPage)masterPage;
                }
                catch (Exception) { }
                return null;
            }
        ...
        {
            string emailSender = "Hệ thống APEM";
            string emailFromAddress = SettingManager.GetSettingValue(SettingNames.ApplicationEmail);
            string emailToAddress = SettingManager.GetSettingValue(SettingNames.ErrorReceiverEmail);
            string subject = string.Format("- Lỗi hệ thống APEM: {0}",
                errorMessage).Replace("\t", " ").Replace("\r", "").Replace("\n", " ");

            StringBuilder strMessage = new StringBuilder();
            strMessage.AppendLine(string.Format("Có lỗi trên hệ thống APEM: {0}", errorMessage));
            strMessage.AppendLine();
            strMessage.AppendLine("-------------------Chi tiết lỗi-------------------");
            strMessage.AppendLine();
            strMessage.AppendLine(string.Format("Tên server: {0}", Server.MachineName));
            strMessage.AppendLine(string.Format("Tên chương trình: {0}", SecurityHelper.APPLICATION_NAME));
            strMessage.AppendLine(string.Format("Người dùng: {0}", ApplicationContext.Current.UserName));
        ...
        public virtual void OpenMessageBox(MessageBox message, ConfirmResult result, bool isClosePostBack, bool showmodal)
        {
        ...
        public virtual void ConfirmRequest(ConfirmResult e)
        {
            if (e.FireInControl)
            {
                BasePage basepage = FindControl(e.ControlName) as BasePage;
                if (basepage != null)
                {
