...
namespace SweetSoft.APEM.WebApp.Common
{
	...
	public static class Code128Rendering
	{
      ...
      private static int ImageWidth(int cursor, int[] codes, int codeLength, double barWeight)
      {
          int result = 0;
          for (int i = 0; i < codeLength; i++)
          {
              ...
              {
                  int barwidth = (int)(cPatterns[code, bar] * barWeight);
                  int spcwidth = (int)(cPatterns[code, bar + 1] * barWeight);
      ...
      public static Image MakeBarcodeImage( string InputData, double BarWeight, bool AddQuietZone, bool AddBarcodeNumber)
      {
         ...
         // skip quiet zone
         int cursor = AddQuietZone ? (int)(cQuietWidth * BarWeight) : 0;

         int width, height;
         width = ImageWidth(cursor, codes, codes.Length, BarWeight);//(int)(( (codes.Length-3) * 11 + 35) * BarWeight);
         height = 45; //Convert.ToInt32(System.Math.Ceiling(Convert.ToSingle(width) * .20F));

         if (AddQuietZone)
         {
             width += 2 * (int)(cQuietWidth * BarWeight);  // on both sides
         }

         // get surface to draw on
         Image myimg = new System.Drawing.Bitmap(width, height + (AddBarcodeNumber ? 27 : 0));
         using (Graphics gr = Graphics.FromImage(myimg))
         {

            // set to white so we don't have to fill the spaces with white
            gr.FillRectangle(System.Drawing.Brushes.White, 0, height, width, 27);
            ...
            for (int codeidx = 0; codeidx < codes.Length; codeidx++)
            {
             ...
             //Add input text to image
            if (AddBarcodeNumber)
            {
                Font font = new Font("Arial", 12, FontStyle.Regular);
                float inputWith = gr.MeasureString(InputData, font).Width;
                float xIndex = width / 2 - inputWith / 2;
                float yIndex = height + 4;
                Graphics graphics = Graphics.FromImage(myimg);
                graphics.DrawString(string.Format("* {0} *",InputData), font, Brushes.Black, xIndex, yIndex);
            }
         }
         return myimg;
      }

      public static string MakeBarcode64BaseImage(string InputData, double BarWeight, bool AddQuietZone, bool AddBarcodeNumber)
      {
          Image myimg = MakeBarcodeImage(InputData, BarWeight, AddQuietZone, AddBarcodeNumber);
