...
Editor.prototype.createOutlineWindow = function() {
    ...
    var btnShowRecord = mxUtils.button(SweetSoftScript.ResourceText.btnShowRecord, function() {
        ...
        ed.editorUi.lastShowProperty = undefined;
    ...
    wnd.addListener(mxEvent.SHOW, function(e) {
        document.getElementById('btnInfo').className = 'btnInfo geButton active';
...
Editor.prototype.isAddNewMainWork = false;
Editor.prototype.renameWindow = undefined;
Editor.prototype.createRenameWindow = function() {
    var table = $('<table class="renamewf">\
        <tr class="row">\
            <td>Tên trục:</td>\
            <td style="padding-right: 20px;"><input type="text" id="txtwfname" style="width: 100%; height: 22px;padding-left: 5px;padding-right: 5px;" /></td>\
        </tr>\
        <tr class="row" id="trradio">\
            <td>Loại trục:</td>\
            <td>\
                <label><input type="radio" style="top: 3px;position: relative;" value="graphic" name="gtype" /><span>Đồ họa</span></label>\
                <label><input type="radio" style="top: 3px;position: relative;" value="truc" checked="checked" name="gtype" /><span>Sản xuất</span></label>\
            </td>\
        </tr>\
        <tr class="row">\
    ...
    var btnok = mxUtils.button(SweetSoftScript.ResourceText.btnApply, function() {
        var newTitle = $('#txtwfname').val();
        if (newTitle.length > 0) {
            if (ed.isAddNewMainWork) {
                var newmainwork = ed.editorUi.sidebar.createItem('-1', newTitle);
                newmainwork.style.height = '25px';
                newmainwork.style.padding = '7px 2px 0px';
                newmainwork.setAttribute('type', $('#trradio input[type="radio"]:checked').val());
                $(newmainwork).text(newTitle);
                mxEvent.addListener(newmainwork, 'click', function(evt) {
                    ed.editorUi.sidebar.showMainWorkDesign(this);
                });
                $('#divtruc').append(newmainwork);
                EditorUi.prototype.currentId = undefined;
                ed.editorUi.callTrigger('click', newmainwork);
            }
            else {
                var activeMainWork = ed.editorUi.sidebar.getActiveMachinery();
                if (activeMainWork != null) {
                    $(activeMainWork).text($('#txtwfname').val());
                }
                ed.editorUi.updateDocumentTitle($('#txtwfname').val());
            }
            ed.renameWindow.setVisible(false);
        }
    });
    btnok.style.marginRight = '10px';
    table.find('tr:eq(2) td:first').append(btnok);

    var btncancel = mxUtils.button(mxResources.get('close'), function() {
        ed.renameWindow.setVisible(false);
    });
    table.find('tr:eq(2) td:first').append(btncancel);

    table.find('#txtwfname').keyup(function(ev) {
    ...
    var wh = 145;
    var ws = SweetSoftScript.mainFunction.getWindowSize();
    var pl = (ws.width - ww) / 2;
    var pt = (ws.height - wh) / 2;
    //mxWindow(title, content, x, y, width, height, minimizable, movable, replaceNode, style)
    var wnd = new mxWindow(SweetSoftScript.ResourceText.renameWindow, table[0], pl, pt, ww, wh, true, true);
    ...
    wnd.addListener(mxEvent.HIDE, function(e) {
        ed.isAddNewMainWork = false;
        document.getElementById('bgrename').style.display = 'none';
    });
    wnd.addListener(mxEvent.SHOW, function(e) {
        var divbg = document.getElementById('bgrename');
        if (divbg === null) {
            ...
            mxUtils.setOpacity(divbg, 70);

            if (mxClient.IS_QUIRKS) {
                new mxDivResizer(divbg);
            }
        ...
        $('#txtwfname').focus();
        if (ed.isAddNewMainWork) {
            $('#trradio input[type="radio"]').removeAttr('disabled');
            $('#txtwfname').val('');
        }
        else {
            $('#trradio input[type="radio"]').attr('disabled', 'disabled');
            //set value
            var activeMainWork = ed.editorUi.sidebar.getActiveMachinery();
            if (activeMainWork != null) {
                $('#txtwfname').val($(activeMainWork).text());

                $('#trradio input[type="radio"][value="'
                    + activeMainWork.getAttribute('type') + '"]').prop('checked', true);
            }
        }
    });
...
Editor.prototype.showRenameWindow = function(isShow) {
    if (this.renameWindow !== null && typeof this.renameWindow !== 'undefined') {
        if (typeof isShow !== 'undefined' && isShow === true)
            this.renameWindow.setVisible(true);
        else
            this.renameWindow.setVisible(!this.renameWindow.isVisible());
    }
    else {
        this.createRenameWindow();
        this.renameWindow.setVisible(true);
    }
};
                            ...
                            if (data.isMain === 1) {
                                ...
                                if (typeof selectprop !== 'undefined') {
                                    if (selectprop.window.isVisible()) {
                                        this.editorUi.lastShowProperty = undefined;
                                        EditorUi.prototype.lastShowProperty = undefined;
