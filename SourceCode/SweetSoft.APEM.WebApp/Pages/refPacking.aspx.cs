...
{
    public partial class refPacking : ModalBasePage
    {
        public override string FUNCTION_PAGE_ID
        {
            get
            {
                return "packing_type_manager";
            }
        }
        ...
        public override void BindData()
        {
            ...
            {
                int totalRows = 0;
                DataTable dt = ReferenceTableManager.SelectAll("", ReferenceTypeHelper.Packing, -1, CurrentPageIndex, gvReference.PageSize, SortColumn, SortType, false);
        ...
        protected void gvReference_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (ReferenceTableManager.ExistCode(ID, code, ReferenceTypeHelper.Packing))
            {
                AddErrorPrompt(txtCountryCode.ClientID, ResourceTextManager.GetApplicationText(ResourceText.EXISTS));
            }
            if (ReferenceTableManager.ExistName(ID, name, ReferenceTypeHelper.Packing))
            {
            ...
            if (IsValid)
            {
                updateRow(e.RowIndex, ID, code, name, ReferenceTypeHelper.Packing, isObsolete);
                gvReference.EditIndex = -1;
                BindData();
        ...
        private void addRow()
        {
            ...
            dr["Code"] = "";
            dr["Name"] = "";
            dr["Type"] = ReferenceTypeHelper.Packing;
            dr["IsObsolete"] = 0;
            dr["RowsCount"] = 0;
        ...
        private void BindData(bool AddNew)
        {
            ...
            {
                int totalRows = 0;
                DataTable dt = ReferenceTableManager.SelectAll("", ReferenceTypeHelper.Packing, -1, CurrentPageIndex, gvReference.PageSize, SortColumn, SortType, AddNew);
                if (dt.Rows.Count == 0 && CurrentPageIndex != 0)
                {
                    CurrentPageIndex -= 1;
                    BindData();
        ...
        private void updateRow(int rowIndex, int ReferencesID, string countryCode, string countryName, byte type, byte isObsolete)
        {
            try
            {
                if (ReferenceTableManager.ExistName(ReferencesID, countryName, ReferenceTypeHelper.Packing))
                {
                    MessageBox msg = new MessageBox(ResourceTextManager.GetApplicationText(ResourceText.DIALOG_MESSAGEBOX_TITLE), "A packing with this information already exists in the system!", MSGButton.OK, MSGIcon.Error);
                    OpenMessageBox(msg, null, false, false);
                    return;
                }
                if (ReferenceTableManager.ExistCode(ReferencesID, countryCode, ReferenceTypeHelper.Packing))
                {
                    MessageBox msg = new MessageBox(ResourceTextManager.GetApplicationText(ResourceText.DIALOG_MESSAGEBOX_TITLE), "A packing with this information already exists in the system!", MSGButton.OK, MSGIcon.Error);
                    OpenMessageBox(msg, null, false, false);
                    return;
