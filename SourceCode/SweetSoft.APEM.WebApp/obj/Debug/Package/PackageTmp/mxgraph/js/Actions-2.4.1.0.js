...
Actions.prototype.init = function() {
    ...
    this.addAction('delete', function() {
        ...
        if (graph.connectionHandler.isConnecting()) {
            ...
            //permissison
            if (graph.isCellDeletable()) {
                //console.log(graph.getSelectionCells());
                var cellSel = graph.getSelectionCells();
                //console.log('cellSel : ', cellSel);
                if (cellSel !== null) {
                    if (cellSel.length === 1) {
                        if (typeof cellSel[0].isSource !== 'undefined' || typeof cellSel[0].isTarget !== 'undefined') {
                            //mxUtils.alert('Thành phần ở bước đầu và bước cuối không thể bị xóa.');
                            SweetSoftScript.mainFunction.OpenRadWindow(undefined, SweetSoftScript.ResourceText.cannotDeleteStartAndEnd, 'alert');
                        }
                        else {
                            var text = cellSel[0].isEdge() ? 'confirmDelete' : 'confirmDeleteName';

                            var obj = '';
                            //console.log(cellSel[0], ui.isMainDocument());
                            if (ui.isMainDocument())
                                obj = SweetSoftScript.ResourceText.dept;
                            else {
                                if (cellSel[0].isSubWork)
                                    obj = SweetSoftScript.ResourceText.subwork;
                                else if (cellSel[0].isSource || cellSel[0].isTarget)
                                    obj = SweetSoftScript.ResourceText.dept;
                                else
                                    obj = SweetSoftScript.ResourceText.worktask;
                            }

                            var data = '';
                            if (cellSel[0].isEdge())
                                data = SweetSoftScript.ResourceText.confirmDeleteLine;
                            else
                                data = mxResources.get(text).replace('{0}', obj).replace('{1}', cellSel[0].getValue());
                            SweetSoftScript.mainFunction.OpenRadWindow(undefined,
                             data,
                             'confirmDelete',
                             function() {
                                 ui.processDeleteCell(cellSel[0]);
                             });
                        }
                    }
                    else {
                        //DEV:not allow delete multi node
                        /*
                        //var data = mxUtils.confirm(mxResources.get('confirmDeleteMulti'));
                        SweetSoftScript.mainFunction.OpenRadWindow(undefined,
                        mxResources.get('confirmDeleteMulti'),
                        'confirmDelete',
                        function() {
                        for (var i = 0, j = cellSel.length; i < j; i++) {
                        ui.processDeleteCell(cellSel[i]);
                        }
                        });
                        */
                        SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.ResourceText.notifyTitle, SweetSoftScript.ResourceText.cannotDeleteMultiNode, 'alert');
                    }
                }
            }
        }
