...
Toolbar.prototype.init = function() {
    var arrfirst = ['undo', 'redo'];
    //permission
    if (this.editorUi.checkEnable())
        arrfirst.push('delete');
    arrfirst.push('-', 'actualSize', 'zoomIn', 'zoomOut', '-');
    this.addItems(arrfirst);
    /*
    ...
    var linestart = this.addMenuFunction('geSprite-startclassic', mxResources.get('linestrat'), false, mxUtils.bind(this, function(menu) {
    ...
    /*DEV:hide edit color, background
    ...
    */
    ...
    btnback.className = 'goback geButton';
    btnback.setAttribute('title', SweetSoftScript.ResourceText.backToMainWork);
    mxEvent.addListener(btnback, 'click', mxUtils.bind(this, function() {
        this.editorUi.showMainDocument();
        if (this.itemSubWork !== null)
            this.itemSubWork.style.display = 'none';
    }));
    this.editorUi.goBackButton = btnback;
    this.container.appendChild(btnback);
    //add back button
    ...
    if (this.editorUi.checkEnable()) {
        ...
        btnsave.setAttribute('href', 'javascript:void(0);');
        btnsave.className = 'btnsave geButton';
        btnsave.setAttribute('title', SweetSoftScript.ResourceText.saveThisDesign);
        mxEvent.addListener(btnsave, 'click', mxUtils.bind(this, function() {
    ...
    //add view outline button
    var btnoutline = document.createElement('a');
    btnoutline.setAttribute('id', 'btnoutline');
    btnoutline.setAttribute('href', 'javascript:void(0);');
    btnoutline.className = 'outline geButton';
    btnoutline.setAttribute('title', SweetSoftScript.ResourceText.viewOutlineDesign);
    mxEvent.addListener(btnoutline, 'click', mxUtils.bind(this, function() {
        this.editorUi.toggleOutLine();
        /*
        var isShow = this.editorUi.outlineWindow.isVisible();        
        if (isShow === false)
        this.className = 'outline geButton hide';
        else
        this.className = 'outline geButton show';
        */
    }));
    this.container.appendChild(btnoutline);
    //add view outline button

    //add reset button
    //permissison
    if (this.editorUi.checkEnable()) {
        if (SweetSoftScript.Setting.reset_workflow === true) {
            var btnreset = document.createElement('a');
            btnreset.setAttribute('id', 'btnreset');
            btnreset.setAttribute('href', 'javascript:void(0);');
            btnreset.className = 'btnreset geButton';
            btnreset.setAttribute('title', SweetSoftScript.ResourceText.resetThisDesign);
            mxEvent.addListener(btnreset, 'click', mxUtils.bind(this, function() {
                this.editorUi.resetDesign();
            }));
    ...
    btnInfo.setAttribute('href', 'javascript:void(0);');
    btnInfo.className = 'btnInfo geButton';
    btnInfo.setAttribute('title', SweetSoftScript.ResourceText.btnInfo);
    mxEvent.addListener(btnInfo, 'click', mxUtils.bind(this, function() {
        this.editorUi.editor.toggleInfo();
        this.editorUi.showInfoCell();
    }));
    this.container.appendChild(btnInfo);
    //add info button

    var ed = this.editorUi;

    //permissison
    /*
    if (this.editorUi.checkEnable()) {
        if (SweetSoftScript.Setting.add_main_workflow === true) {
            //add rename button
            var btnRename = document.createElement('a');
            btnRename.setAttribute('id', 'btnRename');
            btnRename.setAttribute('href', 'javascript:void(0);');
            btnRename.className = 'btnRename geButton';
            btnRename.setAttribute('title', SweetSoftScript.ResourceText.btnRename);
            mxEvent.addListener(btnRename, 'click', mxUtils.bind(this, function() {
                ed.editor.showRenameWindow();
            }));
            this.container.appendChild(btnRename);
            ...
            btnPlus.className = 'btnPlus geButton';
            btnPlus.setAttribute('title', SweetSoftScript.ResourceText.btnPlus);
            mxEvent.addListener(btnPlus, 'click', mxUtils.bind(this, function() {
                ed.editor.isAddNewMainWork = true;
                $('#txtwfname').val('');
                ed.editor.showRenameWindow();
            }));
            this.container.appendChild(btnPlus);
            //add plus button
        }

        if (SweetSoftScript.Setting.delete_workflow === true) {
            //add delete button
            var btnDeleteMW = document.createElement('a');
            btnDeleteMW.setAttribute('id', 'btnDeleteMW');
            btnDeleteMW.setAttribute('href', 'javascript:void(0);');
            btnDeleteMW.className = 'btnDeleteMW geButton';
            btnDeleteMW.setAttribute('title', SweetSoftScript.ResourceText.btnDeleteMW);
            mxEvent.addListener(btnDeleteMW, 'click', mxUtils.bind(this, function() {
                var text = SweetSoftScript.ResourceText.confirmDeleteMainWork;
                SweetSoftScript.mainFunction.OpenRadWindow(undefined,
                 text,
                 'confirmDelete',
                 function() { ed.processDeleteMainWork(); });
            }));
            this.container.appendChild(btnDeleteMW);
            //add delete button
        }
    }
    */
    ...
    //add line type radio
    //var sp = $('<span><input type="radio" value="linemain" id="linemain" name="linetype" checked><label for="linemain">' + SweetSoftScript.ResourceText.rdoLineMain + '</label></span>');
    //divrdo.appendChild(sp[0]);
    /*
    $('#linemain,#linesub').change(function (e) {
    
    //var checked = $(this).prop('checked');
    var node = graph.getSelectionCell();
    if (node && node.isEdge()) {
    var islinesub = this.id === 'linesub';
    if (islinesub === true) {
    graph.setCellStyles(mxConstants.STYLE_STROKECOLOR, SweetSoftScript.Setting.colorSubLine, [node]);
    graph.setCellStyles(mxConstants.STYLE_EDGE, 'elbowEdgeStyle', [node]);
    }
    else {
    graph.setCellStyles(mxConstants.STYLE_STROKECOLOR, '#000000', [node]);
    graph.setCellStyles(mxConstants.STYLE_EDGE, null, [node]);
    }
    ed.saveActionNode(node, 'update');
    }
    });
    */
    /*
    var divrdo = document.createElement('div');
    divrdo.className = 'linetype';
    divrdo.setAttribute('id', 'linetype');

    var sp = $('<span><input type="checkbox" value="linesub" id="linesub" name="linetype"><label for="linesub">' + SweetSoftScript.ResourceText.rdoLineSub + '</label></span>');
    divrdo.appendChild(sp[0]);
    this.container.appendChild(divrdo);
    var graph = this.editorUi.editor.graph;
    var ed = this.editorUi;
  
    $('#linesub').change(function (e) {
    ...
    var ddl = $('<select id="ddllinetype" name="ddllinetype">\
        <option value="1">Đường chuyển chính</option>\
        <option value="2">Đường chuyển phụ</option>\
        <option value="3">Cả hai</option>\
    </select>');
    this.container.appendChild(ddl[0]);
    var graph = this.editorUi.editor.graph;


    $('#ddllinetype').hide().change(function(e) {
        ...
        if (node && node.isEdge()) {
            ...
            switch (val) {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    graph.setCellStyles(mxConstants.STYLE_STROKECOLOR, SweetSoftScript.Setting.colorBothType, [node]);
                    graph.setCellStyles(mxConstants.STYLE_EDGE, 'elbowEdgeStyle', [node]);
                    //graph.setCellStyles('strokeWidth', '1', [node])
                    //editorMain.editor.graph.setCellStyles('strokeWidth','1',[editorMain.lastClickEdge])
                    break;
            }
            ed.saveActionNode(node, 'update');
        }
    ...
    */
    ...
    if (this.editorUi.checkEnable()) {
        //add sub work
        this.editorUi.sidebar.showTooltips = false;
        //backup setting
        var defTW = this.editorUi.sidebar.thumbWidth;
        var defTH = this.editorUi.sidebar.thumbHeight;
        var defshowThumbImage = this.editorUi.sidebar.showThumbImage;
        var defenableTooltips = this.editorUi.sidebar.enableTooltips;
        var defTitleSize = this.editorUi.sidebar.sidebarTitleSize;
        //apply setting
        this.editorUi.sidebar.thumbWidth = 100;
        this.editorUi.sidebar.thumbHeight = 3;
        this.editorUi.sidebar.showThumbImage = false;
        this.editorUi.sidebar.enableTooltips = false;
        this.editorUi.sidebar.sidebarTitleSize = 11;
        //begin add
        var elem = this.editorUi.sidebar.createVertexTemplate('whiteSpace=wrap;fillColor=#FFFF47', 150, 30, '', SweetSoftScript.ResourceText.btnAddSubwork, true);
        elem.className = 'geButton subwork';
        this.itemSubWork = elem;
        this.container.appendChild(elem);
        //restore setting
        this.editorUi.sidebar.thumbWidth = defTW;
        this.editorUi.sidebar.thumbHeight = defTH;
        this.editorUi.sidebar.showThumbImage = defshowThumbImage;
        this.editorUi.sidebar.enableTooltips = defenableTooltips;
        this.editorUi.sidebar.sidebarTitleSize = defTitleSize;
    }
...
/*DEV:subwork*/
Toolbar.prototype.itemSubWork = null;