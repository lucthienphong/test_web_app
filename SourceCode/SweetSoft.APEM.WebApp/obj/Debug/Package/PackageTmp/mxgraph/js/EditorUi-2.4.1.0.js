...
EditorUi = function(editor, container) {
    ...
    var textEditing = mxUtils.bind(this, function(evt) {
        if (evt.target && evt.target.tagName.toLowerCase() === 'select' &&
            evt.target.id && evt.target.id === 'ddllinetype')
            return true;
    ...
    graph.dblClick = function(evt, cell) {
        ...
        if (cell.edge === true || cell.edge === 1) {
            if ((typeof cell.source.isTarget === 'undefined' &&
                typeof cell.source.dataCode === 'undefined' &&
                typeof cell.source.isSource === 'undefined') ||
                (typeof cell.target.isSource === 'undefined' &&
                typeof cell.target.dataCode === 'undefined' &&
                typeof cell.target.isTarget === 'undefined')) {
                return;
            }
            else {
        ...
        if (graph.isEnabled()) {
            ...
            if (st !== null && typeof st !== 'undefined') {
                if (st.cell.isEdge()) {
                    ...
                    else if (curstyle.indexOf(SweetSoftScript.Setting.colorBothType) >= 0) {
                    $('#ddllinetype').val('3');
                    }
                    else {
                    $('#ddllinetype').val('1');
...
EditorUi.prototype.convertToObjSave = function(obj) {
    ...
    var linetype = 'DuongChuyenChinh';
    var st = obj.getStyle();
    if (st && st.length > 0) {
        if (st.indexOf(SweetSoftScript.Setting.colorSubLine) >= 0)
            linetype = 'DuongChuyenPhu';
        if (st.indexOf(SweetSoftScript.Setting.colorBothType) >= 0)
            linetype = 'CaHai';
    }
...
*/
//EditorUi.prototype.splitSize = (mxClient.openIS_TOUCH || mxClient.IS_POINTER) ? 12 : 8;
//DEV:not has split
...
EditorUi.prototype.processOpenWork = function(cell) {
    ...
    if (ta !== null && typeof ta !== 'undefined'
        ...
        if (document.getElementById('btnPlus') !== null)
            document.getElementById('btnPlus').style.display = 'none';
        if (document.getElementById('btnDeleteMW') !== null)
            document.getElementById('btnDeleteMW').style.display = 'none';


        //this.sidebar.hidePaletteByIndex(0);
        this.sidebar.hideAllPalette();
        //this.sidebar.hideMachinery('mainwork');
...
EditorUi.prototype.lastShowProperty = undefined;
    ...
    if (this.loadCellInfoPath.length > 0) {
        ...
        if (typeof dataid !== 'undefined') {
            ...
            new mxXmlRequest(this.loadCellInfoPath, 'idcell=' +
                cell.id + '&idtruc=' + dataid
                + '&wfid=' + (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '')
                + '&type=work').send(onload, onerror);
        }
    }
}
...
EditorUi.prototype.showDetailRecordSubWork = function(info, isSubWork) {
    ...
    if (infowindow !== null && typeof infowindow !== 'undefined') {
        ...
        if (isSubWork) {
            ...
            ww += dx;
            wh += dx;
            left = Math.max(0, Math.round((document.body.scrollWidth - ww) / 2));
            top = Math.max(0, Math.round((Math.max(document.body.scrollHeight, document.documentElement.scrollHeight) - wh) / 3));
        ...
        infowindow.window.setLocation(left, top);
        ...
        if (typeof info !== 'undefined' && info.arr.length > 0) {
            if (div !== null) {
                ...
                for (var i = 0; i < info.arr.length; i++) {
                    var sp = info.arr[i].split('*');
                    var inp;
                    if (this.lastShowProperty) {
                        if (sp[1] === this.lastShowProperty)
                            inp = div.find('input[type="checkbox"][dataid="' + sp[0] + '"]');
                    }
                    else
                        inp = div.find('input[type="checkbox"][dataid="' + sp[0] + '"]');
                    if (inp && inp.length > 0)
            ...
            if (info && info.hasvalue && info.hasvalue === true)
                $('#chkghinhan').prop('checked', info.updatepropertiesvalues);
            else
                $('#chkghinhan').prop('checked', true);
        }
    }
    ...
    if (infowindow !== null && typeof infowindow !== 'undefined') {
        ...
        if (typeof cell !== 'undefined' && cell !== null) {
            ...
            if (typeof info === 'undefined') {
                var ui = this;
                this.loadInfoWork(cell, function(parseData) {
                    info = ui.getInfoWork(cell);
                    ui.showDetailRecordSubWork(info, cell.isSubWork);
                });
            }
            else {
                this.showDetailRecordSubWork(info, cell.isSubWork);
            }
        }
        ...
        if (btn !== null) {
            ...
            if (table.length > 0) {
                ...
                var h3work = $(infowindow.window.td).find('h3.titlework');
                if (h3work.length > 0) {
                    if (this.lastShowProperty) {
                        h3work.text(SweetSoftScript.ResourceText.relatedWork + ' ' + $('#connected .no2 li[dataid="' + this.lastShowProperty + '"] label').text());
                        h3work.show();
                    }
                    else
                        h3work.hide();
...
EditorUi.prototype.arrayUnique = function(array) {
    var a = array.concat();
...
EditorUi.prototype.createProductionProperty = function(id, data) {
    ...
    */
    ...
    var h3work = document.createElement('h3');
    h3work.className = 'titlework';
    divmain.appendChild(h3work);
        ...
        var btnSaveRecord = mxUtils.button(SweetSoftScript.ResourceText.btnSaveRecord, function() {
            ...
            if (chkColl.length > 0) {
                ...
                chkColl.each(function(i, o) {
                    selectedList.push(o.getAttribute('dataid') + '*' + (ed.lastShowProperty || ((ed.lastClickElement && ed.lastClickElement.dataId) || '')));
                });
            }
            if (dataCellInfo) {
                dataCellInfo.hasvalue = true;
                dataCellInfo.updatepropertiesvalues = $('#chkghinhan').prop('checked');
                if (ed.lastShowProperty) {
                    dataCellInfo.arr = dataCellInfo.arr.concat(selectedList);
                    dataCellInfo.arr = ed.arrayUnique(dataCellInfo.arr);
                }
                else
    ...
    wnddesign.addListener(mxEvent.HIDE, function(e) {
        if ($('#connected').closest('div.mxWindow').length > 0)
            $('#connected').closest('div.mxWindow')[0].style.zIndex = '12';
        wnddesign.div.style.zIndex = '-1';
    });
    wnddesign.addListener(mxEvent.SHOW, function(e) {
        if ($('#connected').closest('div.mxWindow').length > 0)
            $('#connected').closest('div.mxWindow')[0].style.zIndex = '1';
        wnddesign.div.style.zIndex = '30';
    });

    wnddesign.addListener(mxEvent.ACTIVATE, function(e) {
        wnddesign.div.style.zIndex = '12';
...
EditorUi.prototype.checkEnable = function() {
    var objEncrypt = this.objEncrypt;
    if (typeof objEncrypt === 'undefined' || objEncrypt === null)
        return false;
    var arrEncrypt = objEncrypt.arrEncrypt;
    var code = objEncrypt.code;
    var newCode = objEncrypt.newCode;
    if (arrEncrypt !== null && typeof arrEncrypt !== 'undefined' && arrEncrypt.length > 0) {
        if (code.length > 0) {
            var checkCode = '';
            var clone = [];
            for (var o = 0; o < arrEncrypt.length; o++)
                clone.push(arrEncrypt[o]);
...
EditorUi.prototype.onBeforeUnload = function() {
    ...
    */
    var activeWF = this.sidebar.getActiveMachinery();
    if (typeof activeWF !== 'undefined' && activeWF !== null) {
        if (activeWF.getAttribute('oldname') !== $(activeWF).text())
            return SweetSoftScript.ResourceText.confirmUnsaveRenamed;
    }
    ...
    if (typeof objectSave !== 'undefined') {
        var isChanged = objectSave.arr.del.length > 0 || objectSave.arr.upd.length > 0;
        if (isChanged)
            return SweetSoftScript.ResourceText.confirmLostData;
        else {
            if (objectSave.arr.ins.length > 0) {
                if (objectSave.arr.ins.length === 2) {
                    if ((objectSave.arr.ins[0].dataCode && objectSave.arr.ins[1].dataCode) ||
                        (objectSave.arr.ins[0].isSource && objectSave.arr.ins[0].isTarget))
                    { }
                    else
                        return SweetSoftScript.ResourceText.confirmLostData;
                }
...
EditorUi.prototype.updateDocumentTitle = function(title) {
    ...
    var activeMachine = this.sidebar.getActiveMachinery();
    if (activeMachine !== null) {
        if (this.editor.getOrCreateFilename() === 'Main-Work')
            title = '[' + $.trim($(activeMachine).text()) + ']';
        else
...
EditorUi.prototype.setHeightDepts = function() {
    ...
    if (divdepts !== null) {
        if (divdepts.parentNode.style.display === 'none')
            ...
            //var arrMainWork = this.sidebar.getMachinery('mainwork');
            //var otherh = arrPanelTruc[0].clientHeight + arrPanelTruc[1].childNodes[0].clientHeight;
            var otherh = 0;
            otherh += arrPanelDepts[0].clientHeight;

            //otherh += arrMainWork[0].clientHeight + arrMainWork[1].childNodes[0].clientHeight;
...
EditorUi.prototype.createOutlineWindowDesign = function() {
    ...
    wnddesign.addListener(mxEvent.SHOW, function(e) {
        document.getElementById('btnoutline').className = 'outline geButton active';
...
EditorUi.prototype.openMainDocument = function(id) {
    var ed = this;

    if (ed.TrucDocument !== null && ed.TrucDocument.length > 0) {
        var title = '';
        var curactive = $('#divtruc .active');
        title = curactive.text();
        if (id) { }
        else
            id = curactive.attr('dataid');

        var data = this.getTrucDocument(id);
        if (typeof data === 'undefined')
            return false;


        if (document.getElementById('btnRename') !== null)
            document.getElementById('btnRename').style.display = 'block';
        if (document.getElementById('btnPlus') !== null)
            document.getElementById('btnPlus').style.display = 'block';
        if (document.getElementById('btnDeleteMW') !== null)
            document.getElementById('btnDeleteMW').style.display = 'block';
        ...
        ed.updateDocumentTitle(title.length > 0 ? title : SweetSoftScript.ResourceText.mainworkTitle);
        ...
        //ed.sidebar.showMachinery('mainwork');
        //ed.sidebar.showPalette('depts');
        //ed.sidebar.setHeightVisiblePalette();
        ed.setHeightDepts();
        return true;
    }
    return false;
...
EditorUi.prototype.processLoadDraw = function(target, source) {
    ...
    if (typeof dataText === 'undefined') {
        ...
        //    EditorUi.prototype.lastParentId = EditorUi.prototype.currentId;
        EditorUi.prototype.currentId = obj.id.toString();
        if (obj.source && obj.source.idparent)
            EditorUi.prototype.lastParentId = obj.source.idparent;
        EditorUi.prototype.lastWorkDocument = name;
...
EditorUi.prototype.processDeleteMainWork = function() {
    var ui = this;
    var dataid = this.getDataId();
    var code = EditorUi.prototype.lastWorkDocument;
    if (typeof code === 'undefined' || code.length === 0)
        ...
        if (typeof data !== 'undefined' && data !== null && data.length > 0) {
            try {
                ...
                if (typeof parseData !== 'undefined' && parseData.isError === false) {
                    var activemw = ui.sidebar.getActiveMachinery();
                    if (activemw !== null)
                        $(activemw).remove();
                    var firstmw = $('#divtruc .geItem:first');
                    if (firstmw.length > 0)
                        ui.callTrigger('click', firstmw[0]);
                    ...
                    document.getElementById('page_spinner').style.display = 'none';
                    SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.ResourceText.notifyTitle, parseData.errorMessage, 'alert');
                }
            }
            catch (ex) {
                document.getElementById('page_spinner').style.display = 'none';
                SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.ResourceText.notifyTitle,
                    SweetSoftScript.ResourceText.alertErrorSave, 'alert');
            }
        }
        else {
            document.getElementById('page_spinner').style.display = 'none';
    ...
    new mxXmlRequest(ui.deletePath, 'act=delete&wfid=' +
           (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '') +
        '&filename=' + code + '&idtruc=' + dataid).send(onload, onerror);
}
...
EditorUi.prototype.processDeleteCell = function(cell) {
    if (this.editor.graph.isCellDeletable()) {
        ...
        */
            ...
            if (typeof data !== 'undefined' && data !== null && data.length > 0) {
                ...
                try {
                    if (typeof parseData !== 'undefined' && parseData.isError === false) {
                        ...
                        if (cell.isEdge()) {
                            var arr = ui.externalType;
                            var arrNodeTruc = undefined;
                            if (arr !== null && typeof arr !== 'undefined') {
                                var isgraphic = true;
                                for (var i = 0; i < arr.length; i++) {
                                    if (cell.source.dataId && arr[i].sid.toString() === cell.source.dataId.toString()
                    ...
                    document.getElementById('page_spinner').style.display = 'none';
                    SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.ResourceText.notifyTitle, SweetSoftScript.ResourceText.alertErrorSave, 'alert');
                }
            }
            else {
                document.getElementById('page_spinner').style.display = 'none';
        ...
        new mxXmlRequest(ui.deletePath, 'objId=' + cell.id + '&wfid=' +
               (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '') +
            '&filename=' + code + ('&isedge=' + (cell.isEdge() ? '1' : '0'))
                ).send(onload, onerror);
        /*}
        else {
        ...
        try {
            if (useLocalStorage) {
                ...
                if (xml.length < MAX_REQUEST_SIZE) {
                    ...
                    var dataid = this.getDataId();
                    var title = '';
                    var curmw = this.sidebar.getActiveMachinery();
                    if (typeof curmw !== 'undefined'&&curmw!==null) {
                        if (curmw.getAttribute('oldname') !== $(curmw).text())
                            title = encodeURIComponent($(curmw).text());
                    }
                    var ed = this;
                    ...
                    var onload = function(req) {
                        ...
                        if (data === 'login') {
                            ed.resetActionNode();
                            location.href = '/pages/AccessDenied.aspx';
                        }
                        else {
                            if (ed.mainDocument !== null && name === ed.mainDocument.id.toString()) {
                                ...
                                if (typeof data !== 'undefined' && data.length > 0
                                    ...
                                    if ($('#divtruc a[dataid="-1"].active').length > 0) {
                                        $('#divtruc a[dataid="-1"].active').attr('dataid', data.split('||')[2]);
                                    }
                                    ...
                                    if (title.length > 0)
                                        curmw.setAttribute('oldname', decodeURIComponent(title));
                                    ...
                                    if (showMessage !== false) {
                                        //show success save                         
                                        SweetSoftScript.mainFunction.OpenRadWindow(SweetSoftScript.ResourceText.notifyTitle, data.split('||')[0], 'alert');
                                        //if ((typeof (EditorUi.prototype.currentId) !== 'undefined') && EditorUi.prototype.currentId !== null)
                                        //    EditorUi.prototype.lastParentId = EditorUi.prototype.currentId;
                                    ...
                                    if (data && data.length > 0) {
                                        try {
                                            ...
                                            SweetSoftScript.mainFunction.OpenRadWindow(undefined, data, 'alert');
                    ...
                    var typedraw = '';
                    if (title.length > 0) {
                        typedraw = curmw.getAttribute('type');
                        if (typedraw === 'truc')
                            typedraw = 'machinery';
                    }
                    new mxXmlRequest(SAVE_URL, 'filename=' + (name.toLowerCase().indexOf('main-work') >= 0 ? 'Main-Work' : name) + '&xml=' + encodeURIComponent(xml)
                        + '&title=' + title + '&type=' + typedraw + '&wfid='
                        + (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '')
                        + '&idparent=' + (typeof EditorUi.prototype.lastParentId !== 'undefined' ? EditorUi.prototype.lastParentId : 0)
                        ...
                        //+ ((typeof EditorUi.prototype.IdOfLineMap !== 'undefined') ? ('&lineid=' + EditorUi.prototype.IdOfLineMap) : '')
                        ).send(onload, onerror);
                }
    ...
    if (this.loadCellInfoPath.length > 0) {
        ...
        if (typeof dataid !== 'undefined') {
            if (cell.isSubWork === '1' || cell.isSubWork === 1) {
                ...
                new mxXmlRequest(this.loadCellInfoPath, 'idcell=' + cell.id
                      + '&wfid=' + (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '')
                      + '&idtruc=' + dataid + '&filename=' + code + '&type=subwork').send(onload, onerror);
            }
        }
    }
}
...
EditorUi.prototype.getDataId = function() {
    var dataid = '0';

    var activeMachine = this.sidebar.getActiveMachinery();
    //if (activeMachine === null)
    //    activeMachine = this.sidebar.getActiveMainWork();
    if (activeMachine !== null)
...
EditorUi.prototype.processSaveInfoCell = function(objsave, dataid) {
    ...
    if (arrSub.length > 0 || arrWork.length > 0) {
        ...
        if (arraySubSave.length > 0 || arrayWorkSave.length > 0) {
            ...
            new mxXmlRequest(ed.saveCellInfoPath,
            /*'filename=' + code +
            '&IdTruc=' + dataid +*/
                    'idwf=' + (typeof EditorUi.prototype.currentId !== 'undefined' ? EditorUi.prototype.currentId : '') +
...
EditorUi.prototype.processShowSelectWork = function() {
        ...
        if (workcoll.length > 0) {
            ...
            if (typeof dataCell !== 'undefined') {
                ...
                if (dataCell.arr !== null && typeof dataCell.arr !== 'undefined' &&
                    ...
                    var isshowmapp = ed.checkProductionProperty();
                    for (var i = 0; i < dataCell.arr.length; i++) {
                        ...
                        if (pos !== -1) {
                            ...
                            selected += '<li dataid="' + workcoll[pos].getAttribute('dataid') + '"><label><input type="checkbox"><span>' +
                                $.trim($(workcoll[pos]).text()) + '</span></label>' +
                                (isshowmapp ? '<img src="images/outline.png"/>' : '') +
                                '<div class="clear"></div></li>';
                        }
                    }
                }
                if (selected.length > 0) {
                    selected = '<li><label><input type="checkbox" id="deselectall"><span>' + SweetSoftScript.ResourceText.btnSelectAll + '</span></label><div class="clear"></div></li>' + selected;
                    ...
                    $(div).find('.connected:eq(1) > li img').click(function() {
                        ed.lastShowProperty = $(this).closest('li').attr('dataid');
                        EditorUi.prototype.lastShowProperty = $(this).closest('li').attr('dataid');
                        ed.showProductionProperty();
                    });
    ...
    $(div).find('#btnSelect').click(function() {
        ...
        if (selected.length > 0) {
            if ($('#connected .connected:eq(1) > li').length === 0) {
                var liall = $('<li><label><input type="checkbox" id="deselectall"><span>' + SweetSoftScript.ResourceText.btnSelectAll + '</span></label><div class="clear"></div></li>');
            ...
            selected.each(function() {
                ...
                if (isshowmapp)
                    pa.append('<img src="images/outline.png"/><div class="clear"></div>');
                else
                    pa.append('<div class="clear"></div>');
                pa.appendTo($('#connected .connected:eq(1)'));
            });

            $('#connected .connected:eq(1) > li img').click(function() {
                ed.lastShowProperty = $(this).closest('li').attr('dataid');
                EditorUi.prototype.lastShowProperty = $(this).closest('li').attr('dataid');
                ed.showProductionProperty();
            });
    ...
    $(div).find('#btnRemoveSelect').click(function() {
        ...
        if (selected.length > 0) {
            ...
            var arrdel = [];
            selected.each(function() {
                arrdel.push($(this).closest('li').attr('dataid'));
                $(this).prop('checked', false).parent().parent().find('.clear,img').remove().end().appendTo($('#connected .connected:eq(0)'));
            });

            if (arrdel.length > 0) {
                var info = ed.getInfoWork(ed.lastClickElement);
                if (info && info.arr.length > 0) {
                    var newarr = [];
                    for (var ii = 0; ii < info.arr.length; ii++) {
                        if ($.inArray(info.arr[ii].split('*')[1], arrdel) === -1)
                            newarr.push(info.arr[ii]);
                    }
                    info.arr = newarr;
                    ed.saveInfoWork(ed.lastClickElement, info);
                }
            }
