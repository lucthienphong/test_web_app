...
function Dialog(editorUi, elt, w, h, modal, closable, onClose) {
    ...
    var top = (SweetSoftScript.mainFunction.getWindowSize().height - h) / 2;
...
function SubworkDialog(editorUi, elem, w, h, onClose) {
    var dx = 0;
    ...
    var wnd = new mxWindow('Chọn công việc ' + EditorUi.prototype.lastClickElement.value, elem, left, top, w, h, true, true);
    ...
    var dg = this;
    wnd.addListener(mxEvent.DESTROY, function(e) {
        if (dg.onDialogClose !== null) {
            dg.onDialogClose();
            dg.onDialogClose = null;
        }
        
        dg.bg.parentNode.removeChild(dg.bg);
    });
    wnd.addListener(mxEvent.ACTIVE, function(e) {
        wnd.div.style.zIndex = '12';
    });
    dg.onDialogClose = onClose;
    wnd.destroyOnClose = true;

    //wnd.setTitle(title);

    if (typeof dg.bg === 'undefined' || this.bg === null) {
        dg.bg = editorUi.createDiv('background');
        dg.bg.style.position = 'absolute';
        dg.bg.style.background = 'white';
        dg.bg.style.left = '0px';
        dg.bg.style.top = '0px';
        dg.bg.style.bottom = '0px';
        dg.bg.style.right = '0px';
        dg.bg.style.zIndex = '1';

        mxUtils.setOpacity(dg.bg, dg.bgOpacity);

        if (mxClient.IS_QUIRKS) {
            new mxDivResizer(dg.bg);
        }
    }

    document.body.appendChild(dg.bg);

    /*
    
    $('#connected .connected').sortable({
    connectWith: '#connected .connected'
    }).bind('dragstart.h5s', function (e, ui) {
    //ui.item contains the current dragged element.
    //Triggered when the user stopped sorting and the DOM position has changed.
    $('#connected .connected.no2').addClass('tothis');
    }).bind('dragend.h5s', function (e, ui) {
    //ui.item contains the current dragged element.
    //Triggered when the user stopped sorting and the DOM position has changed.
    $('#connected .connected.no2').removeClass('tothis');
    });
    */
...
SubworkDialog.prototype.bgOpacity = 80;
SubworkDialog.prototype.bg = undefined;
SubworkDialog.prototype.onDialogClose = undefined;
SubworkDialog.prototype.close = function() {
