...
namespace SweetSoft.APEM.Core.Helper
{
	public class WebControlAdapterExtender
	{
		...
		{
			/* 
			 * Modified for support of compiled NotAClue.Web.BootstrapFriendlyControlAdapters assembly
			 * 
			 * We will first search for embedded JavaScript files. If they are not
			 * found, we default to the standard approach.
			 */

			Type type = typeof(WebControlAdapterExtender);

			// AdapterUtils.js
			Helpers.RegisterClientScript("NotAClue.Web.BootstrapFriendlyControlAdapters.JavaScript.AdapterUtils.js", type, AdaptedControl.Page);
		}
